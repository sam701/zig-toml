...
test "value array" {
    const St = struct { values: []const Value };
    var reader = std.Io.Reader.fixed(
...
test "int hashmap" {
    const St = struct { map: std.StringHashMapUnmanaged(i32) };
    var reader = std.Io.Reader.fixed(
        \\ [map]
        \\ aa = 4
        \\ bb = 5
    ...
    try expectEqual(4, result.value.map.get("aa").?);
    try expectEqual(5, result.value.map.get("bb").?);
}
